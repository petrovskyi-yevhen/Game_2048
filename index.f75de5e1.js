...
function move(e) {
    ...
    for(let row = 0; row < size; row++){
        for(let col = 0; col < size; col++)if (newBoard[row][col] !== board[row][col]) {
            board = newBoard;
            addNumber();
            render();
        }
...
function transposeGameField() {
    newBoard = newBoard[0].map((_, colIndex)=>newBoard.map((row)=>row[colIndex]));
}
...
function checkRows() {
    for(let i = 0; i < size; i++){
        if (newBoard[i].some((cell)=>cell === 0) || newBoard[i].some((cell, j)=>cell === newBoard[i][j + 1])) return true;
    }
    return false;
}
function checkColumns() {
    for(let i = 0; i < size; i++){
        if (newBoard[i].some((cell, j)=>cell === newBoard[i][j + 1])) return true;
    }
    return false;
}
